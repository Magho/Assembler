...
list<map<string, string>> ParsingMap :: parisngFunction (string path) {
...
size_t ParsingMap :: split(const std::string &txt, std::vector<std::string> &strs, char ch) {
...
bool ParsingMap :: startsWith (string line, string charsStartWith) {
    if(line.length() < charsStartWith.length())
        return false;
    return (line.compare(0, charsStartWith.length(), charsStartWith) == 0);
}

void ParsingMap :: printTheList (list<map<string, string>> aaaaa) {
    while ( aaaaa.size() != 0) {
        map<string, string> bbbb;
        bbbb = aaaaa.front();
        aaaaa.pop_front();
